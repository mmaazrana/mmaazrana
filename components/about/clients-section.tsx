        ...
        <div className='flex flex-col gap-2 sm:gap-3 md:gap-4 h-fit group'>
          <div className='flex flex-row flex-wrap gap-y-m gap-x-s relative group'>
            {clientIconComponents.map(({ Icon }, index) => {
              ...
              if (mounted) {
                if (theme === 'light') {
                  themeSpecificClasses = '!mix-blend-darken'
                } else if (theme === 'dark') {
                  themeSpecificClasses = '!mix-blend-color-dodge'
                }
              }
              ...
              return (
                <div key={index} className='relative'>
                  <div
                    className={`absolute inset-0 bg-primary-accent mix-blend-darken dark:mix-blend-color-dodge z-1 ${themeSpecificClasses}`}
                  />
                  <div

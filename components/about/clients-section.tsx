          ...
          <div className='flex flex-row flex-wrap gap-y-m gap-x-s relative group'>
            {clientIconComponents.map(({ Icon }, index) => (
              <div key={index} className='relative'>
                <div
                  className={`absolute inset-0 bg-primary-accent mix-blend-darken dark:mix-blend-color-dodge z-1 ${theme === 'light' && '!mix-blend-darken'} ${theme === 'dark' && '!mix-blend-color-dodge'} ${isActive ? 'opacity-0' : 'opacity-100'} group-hover:opacity-0`}
                />
                <div

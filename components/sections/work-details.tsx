            ...
            content={
              <div className={'flex flex-col gap-6'}>
                <div className={'flex flex-col gap-8'}>
                  <Typography type={TextTypes['2xl']}>{data.testimonial}</Typography>
                  <div className={'flex justify-end items-center gap-2'}>
                    <Typography type={TextTypes.xl} weight={WeightTypes.bold}>
                      {data.clientName}
                    </Typography>
                    <Typography type={TextTypes.xl}>-</Typography>
                    <Typography type={TextTypes.xl}>{data.companyName}</Typography>
                  </div>
                </div>
                <div className={`flex flex-col gap-12`}>
                  <div className={'flex flex-col gap-10'}>
                    ...
                    {data.productsInvolved.map((product, index) => (
                      <div key={index} className={'flex justify-between items-center ml-12'}>
                        ...
                        <div className={'flex flex-row justify-end items-center gap-2 '}>
                          {product.services.map((service, index) => (
                            <div
                              key={index}
                              className={'flex px-5 py-2 rounded-full bg-secondary-hover'}
                            >
                              <Typography type={TextTypes.lg}>{service}</Typography>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  <div className={'flex flex-col gap-10'}>
                    ...
                    <div className={'flex flex-row justify-start items-center gap-8 ml-12'}>

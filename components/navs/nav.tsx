...
const Nav: FC<NavProps> = ({ setInView }) => {
          ...
          <div className=" flex items-center justify-between md:block  ">
            ...
            <div className=" md:hidden ">
              <button
                ...
                className=" grid justify-center items-end outline-0 p-2 ${navbar ? 'items-start' : 'items-end'}`}"
              >
                <span
                  className={`block transition-all duration-300 ease-out h-0.5 w-5 rounded-sm ${navbar ? "bg-error rotate-45 translate-y-1 w-3   " : "bg-primary -translate-y-0.5 w-5"}`}
                ></span>
                <span
                  className={`bg-primary transition-all duration-300 ease-out h-0.5 w-5 rounded-sm my-0.5 ${navbar ? "opacity-0" : "opacity-100"}`}
                ></span>
                <span
                  className={` block transition-all duration-300 ease-out h-0.5 w-3 rounded-sm ${navbar ? "bg-error -rotate-45 -translate-y-1 w-5  " : "bg-primary translate-y-0.5 w-3"}`}
                ></span>
              </button>
            </div>
          </div>

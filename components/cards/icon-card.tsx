...
import { renderChildrenWithClassName } from '@/helpers'
...
export default function IconCard({ children, title, description }: IconCardProps) {
  ...
  return (
    <div ref={cardRef} className='relative group'>
      {renderChildrenWithClassName(
        children,
        `self-end top-6 sm:-top-4 -left-6 sm:left-auto sm:-right-8 absolute w-10xl h-10xl stroke-[0.5px] text-secondary opacity-0 group-hover:opacity-75 group-hover:-translate-y-16 transition-all duration-300 ease-in-out ${cardIsInView && isMobile && 'opacity-75 -translate-y-16'}`,
      )}
      <div
      ...
      >
        ...
        {renderChildrenWithClassName(
          children,
          'self-end top-4 xs:top-5 sm:top-0 right-4 xs:right-5 sm:right-0 absolute sm:relative sm:self-start w-5xl h-5xl stroke-1 text-secondary',
        )}
